...
Seeking $2M Series A to scale user acquisition and expand enterprise features.`,
    authorId: "default-author", // You'll need to replace this with an actual author ID
    views: 0
  },
  {
...
async function seedStartups() {
  try {
    ...
    if (authors.length === 0) {
      ...
      // Update all sample startups to use this author
      sampleStartups.forEach(startup => {
        startup.authorId = defaultAuthor._id;
      });
    } else {
      // Use the first available author
      sampleStartups.forEach(startup => {
        startup.authorId = authors[0]._id;
      });
